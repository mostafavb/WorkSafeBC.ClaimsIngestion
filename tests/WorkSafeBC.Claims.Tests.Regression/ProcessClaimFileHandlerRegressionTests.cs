...
public sealed class ProcessClaimFileHandlerRegressionTests
{
    ...
    [Fact]
    public async Task HandleAsync_ShouldQueueReview_WhenValidationThrows()
    {
        var parser = new Mock<IClaimFileParser>(MockBehavior.Strict);
        var validator = new Mock<IClaimBusinessValidator>(MockBehavior.Strict);
        var publisher = new Mock<IClaimMessagePublisher>(MockBehavior.Strict);
        ...
        reviewInbox.Setup(x => x.QueueAsync(It.IsAny<ClaimReviewItem>(), It.IsAny<CancellationToken>()))
            ...
            .Returns(Task.CompletedTask);
        ledger.Setup(x => x.MarkReviewRequiredAsync(file.FileName, "Claim amount must be greater than zero.", It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
    ...
    [Fact]
    public async Task HandleAsync_ShouldQueueReview_WhenPublisherThrows()
    {
        var parser = new Mock<IClaimFileParser>(MockBehavior.Strict);
        var validator = new Mock<IClaimBusinessValidator>(MockBehavior.Strict);
        var publisher = new Mock<IClaimMessagePublisher>(MockBehavior.Strict);
